...
class MazeSolver:
    ...
    def reduce_positions(self):
        for _ in range(20):  # Perform 20 random moves
            direction_key = random.choice(list(self.directions.keys()))  # Select a random direction by key
            ...
            self.moves_taken += direction_key  # Append the move to the moves string
        ...
        while queue:
            ...
            for direction_key, (dx, dy) in self.directions.items():
                ...
                if new_positions_frozen not in self.visited:
                    if len(new_positions) < min_positions:
                        min_positions = len(new_positions)
                        queue.clear()
